...
for i,secao in enumerate(secoes, start=1):
    titulo,conteudo = secao.split('\n', maxsplit=1)
    nome_arq = f"{i:02d}-{slugify(titulo)}.md"
    with open(nome_arq, mode='w', encoding='utf-8') as arq:
        arq.write(f"# {i:02d}. {titulo}\n{conteudo}")
%hist
%hist -f zensical-tutor-split.py
